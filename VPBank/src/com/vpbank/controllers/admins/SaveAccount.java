...
public class SaveAccount extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // if account with similar basic number exists, generate random prefix
        // number
        ...
        // IBAN number
        String accountNumberIBAN = AccountGenerator.genAccountNumberIBAN(accountNumberBasic, accountNumberPrefix,
                bankCode, "SK");
        account.setAccountNumberIBAN(accountNumberIBAN);

        // account name
        // get string form of the accountNumberPrefix
        // 4 in the next line means length of country code (2 symbs) + 2 check
        // digits
        String strAccountNumberPrefix = accountNumberIBAN.substring(4 + strBankCode.length(),
        ...
        calOpen.set(Calendar.MONTH, Integer.parseInt(month) - 1); // because
                                                                  // months 0-11
        ...
        calClose.set(Calendar.MONTH, Integer.parseInt(month) - 1); // because
                                                                   // months
                                                                   // 0-11
        ...
        int clientId = client.getId(); // take id of client for using in
                                       // searching in DB

        // add account to client's list of accounts
        this.as.addAccountToClient(account, clientId);
    }
